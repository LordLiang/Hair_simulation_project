...
inline float Mp(float b, float theta_i, float theta_o) 
{
    ...
    //float b_ = exp(sin(-theta_i) * sin(theta_o) / v);
    //float c = I0(cos(-theta_i) * cos(theta_o) / v);
    //return a * b_ * c;
    return g(b, 2*(theta_o + theta_i));
}
...
inline half4 light_scattering_hair2(in sampler2D smap, float3 light_color, float3 ambient_color, float3 i, float3 o, float3 u, float kd, float ks)
{
    const float PI = 3.141592653589793f;
    const float PI2 = 2 * PI;
    const float ETA = 1.55f;
    ...
    float2 uv = float2(2 * abs(theta_d) / PI, abs(phi) / PI);
    float3 n = tex2D(smap, uv).rgb;

    //diffuse term
    s += kd * tex2D(smap, float2(0.0f, 1.0f)).rgb * light_color * max(0.0f, 0.75 * cos_theta_i * cos_half_phi + 0.25f);

    //R term
    float mr = Mp(BETA_R, theta_i, theta_o - ALPHA_R);
    //float nr = fresnel(ETA, sqrt(dot(i, o) * 0.5f + 0.5f)) * 0.25 * cos_half_phi;
    s += ks * light_color * mr * n.r;
