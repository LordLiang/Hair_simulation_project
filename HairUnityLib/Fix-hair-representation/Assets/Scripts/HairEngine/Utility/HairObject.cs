    ...
    namespace Utility {
        ...
        public class HairObject: MonoBehaviour {
            ...
            public enum HairColorStyle {
                Rainbow = 0 //show the hair in rainbow color
            }
			...
			private float currentHairRadius = 0.0005f;
			...
			public float CurrentHairRadius
            {
                ...
                set
                {
                    ...
                    //{
                    //    subhair.GetComponent<MeshRenderer>().material.SetFloat("_R", value);
                    //}
                }
            }
            ...
            public void SetHairRepresentationStyle(HairColorStyle style) {
                if (style == HairColorStyle.Rainbow)
                    SetHairRepresentationStyleToRainbow();
            }
             
            //set the hair style to rainbow color
            private void SetHairRepresentationStyleToRainbow() {
                const float split = 2.0f;

                //we first random select a hair end particle as our start point, then determine each hair's color
                //from the hair end distance to the start point
                var vertices = GetVerticesBuffer();

                //in order to have the same hair style representation if one has the same hair geometry
                Vector3 origin = new Vector3(1e30f, 1e30f, 1e30f);
                for (int i = particleCount / 2, round = 0; !(i == particleCount / 2 && round > 0);) {
                ...
                if (origin.x >= 1e30f)
                    throw new Exception("no hair");

                //get the largeast distance from the hair root
                float maxDistance = 1e-30f;
                foreach (int i in hairEndParticleSet) {
                    var position = vertices[i];
                    var distance = Vector3.Distance(position, origin) + 1e-30f;
                ...
                foreach (Transform subhair in this.transform) {
                    ...
                    for (int i = indexer.particleBeginIndex; i < indexer.particleEndIndex; ++i) {
                        if (IsHairEndParticle(i)) {
                            var ratio = Vector3.Distance(vertices[i], origin) / maxDistance * split;
                            color = new Color(65.0f / 255.0f, 40.0f / 255.0f, 32.0f / 255.0f);
                            //color = Color.HSVToRGB(ratio - Mathf.Floor(ratio), 0.78f, 0.9f);
