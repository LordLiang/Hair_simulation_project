...
Shader "Custom/HairStrand" {
	Properties{
		_R("Hair Stroke Width", Float) = 0.0003
        _Color("Hair Color", Color) = (0.604, 0.384, 0.310, 1.0)
        _KDiffuse("Diffuse Term", Range(0.0, 3.0)) = 0.8
        _KR("R Term", Range(0.0, 3.0)) = 0.8
        _KTT("TT Term", Range(0.0, 3.0)) = 0.42
        _KTRT("TRT Term", Range(0.0, 3.0)) = 2.00
	}
	...
	SubShader {
		...
		Pass {
			...
			half4 frag(PS_INPUT p) : SV_TARGET{
				...
				return light_scattering_hair(_Color, light_color, ambient, light_dir, view_dir, p.world_tan, _KDiffuse, _KR, _KTT, _KTRT);
			}
